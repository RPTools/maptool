...
public class ExpressionParserTest extends TestCase {
    ...
    private void evaluateExpression(ExpressionParser p, String expression, BigDecimal answer) throws ParserException {
        Result result = p.evaluate(expression);
        assertTrue(String.format("%s evaluated incorrectly expected <%s> but was <%s>", expression, answer, result.getValue()),
                answer.compareTo((BigDecimal) result.getValue()) == 0);
    }

    private void evaluateStringExpression(ExpressionParser p, String expression, String answer) throws ParserException {
        Result result = p.evaluate(expression);
