...
/**
 * An object of this class is created in the {@link PersistenceUtil} code for the purpose of applying XML transforms to data being read or written by a tool (MapTool). The application registers one or
 * more {@link ModelVersionTransformation} objects under a particular version. When the {@link #transform(String, String)} method is called with an XML string and a version number string, all
 * transforms currently registered will be searched and those with a version number earlier than the one passed in will have their <code>transform</code> method called.
 * <p>
 * For example, a version number of "1.3.53" registered with a manager means that only versions later than that will apply the given transform. So version "1.3.54" would have the transformation
 * performed as would version "1.3.53.1". Note that version numbers are cleaned (via {@link #cleanVersionNumber(String)} prior to being compared.
 * <p>
 ...
 * </ul>
 * <p>
 * Those can be mitigated somewhat by performing an upfront check of the file version number against the version numbers stored in this object. If there are no transforms required then the alternate
 * code in {@link PackedFile} that uses InputStreams and Readers can be used instead, significantly reducing the memory footprint of the import process.
 * 
 * @author tcroft
 */
public class ModelVersionManager {
	...
	/**
	 * Compare the given version number (expected to be read from a file about to be imported) against the version numbers that have been registered with this transformation so far and return
	 * <code>true</code> only if later version numbers have been registered.
	 * <p>
	 * Essentially this allows the calling code to skip the step of reading the XML into a String, resulting in better performance since the InputStream or Reader can be passed directly to the XStream
	 * library. And because this will be the common case, it should typically improve performance.
	 * 
	 * @param fileVersion
	...
	/**
	 * Registers one or more transformations to execute when a campaign file earlier than the supplied version number is being processed. For example, if the version parameter were <b>1.3.66</b> then
	 * the list of transforms would run for any version before that, such as <b>1.3.64.1</b> or <b>1.3.65</b>.
	 * 
	 * @param version
	...
	/**
	 * Tests whether the first parameter is a version number that comes before the version number stored in the second parameter. Version numbers are of the form <code>1.3.51</code> or
	 * <code>1.3.64.1</code> and may have any number of components separated by periods. If either parameter contains a component which cannot be parsed as an integer, the component is treated as
	 * <code>0</code> for the purposes of this comparison. Also, if a parameter has fewer components than the other, it is considered to have <code>0</code> in the place of each missing component.
	 * 
	 * @param version1
	...
	/**
	 * Removes any characters from a version string other than digits and periods. If cleaning results in an empty value, <code>"0"</code> is returned instead.
	 * 
	 * @param version
