 ...
 *
 * <p>
 * As MapTool uses a base name for the string and extensions for alternate pieces (such as <code>
...
public class I18N {
     ...
     * the <b>Action</b>. This method uses the same key as used by the prior methods, but appends
     * the string <code>ACCELERATOR_EXTENSION</code> to the end. This allows a single key to be
     * composed of multiple (optional) parts.
     *
     * <p>
     * Note that the modifier key used by the platform to initiate the sequence is not included in
     * the properties file. This is because the key changes from platform to platform. For example,
     * on Windows the key is the Control key, while on Mac OSX the key is the Command key. The AWT
     ...
     * The application does this by retrieving the platform's <code>menuShortcut</code> and saving
     * it in a <code>final</code> field. The value of that field is used as the modifier throughout
     * the application. The properties file may still specify other modifiers, such as "Shift" or
     * "Alt".
     *
     * <p>
     * This seems a little bogus, though. Shouldn't <b>all</b> keystrokes be definable from outside
    ...
    /**
     * Returns the description text for the given key. This text normally appears in the statusbar
     * of the main application frame. As described by the {@link #getAccelerator(String)} method,
     * the input key has the string DESCRIPTION_EXTENSION appended to it.
     *
     * @param key
     * @return
     */
    ...
    /**
     * Returns the text associated with the given key after removing any menu mnemonic. So for the
     * key <b>action.loadMap</b> that has the value "&Load Map" in the properties file, this method
     * returns "Load Map".
     *
     * @param key
     *            the component to search for
     ...
     * example usage. Full documentation for this technique can be found under
     ...
     *
     * <p>
     * Uses the <code>key</code> as the index for the properties file to set the <b>Action.NAME</b>
     * field of <b>action</b>.
     *
     * <p>
     * The string used for the <b>NAME</b> is searched for an ampersand ("&amp;") to determine the
     * mnemonic used by any menu item (no mnemonic is set if there is no ampersand). If there is
     * one, the <b>Action.MNEMONIC_KEY</b> property is set.
     *
     * <p>
     * The <code>key</code> string has "<code>.accel</code>" appended to it and the properties file
     * is searched again, this time to obtain a string representing the shortcut key. If there is
     * one, the <b>Action.ACCELERATOR_KEY</b> property is set.
     *
     * <p>
     * The <code>key</code> string has "<code>.description</code>" appended to it and the properties
     * file is searched again, this time to obtain a string representing the status bar help
     * message. If there is one, the <b>Action.SHORT_DESCRIPTION</b> property is set.
     *
     * <p>
     * If <b>addMenuShortcut</b> is <code>true</code> then the proper shortcut key for the platform
     ...
     *            whether to add the platform's menu shortcut key mask (usually <code>true
     *     </code>)
     */
