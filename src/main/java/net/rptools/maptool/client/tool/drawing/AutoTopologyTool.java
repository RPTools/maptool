...
public class AutoTopologyTool extends HollowRectangleTopologyTool {
    ...
    public AutoTopologyDialog(int x1, int x2, int y1, int y2) {
      ...
      sourceImg =
          img.getSubimage(
              (int) sp1.getX(),
              (int) sp1.getY(),
              (int) (sp2.getX() - sp1.getX()),
              (int) (sp2.getY() - sp1.getY()));
      thresholdImg =
          new BufferedImage(
              sourceImg.getWidth(), sourceImg.getHeight(), BufferedImage.TYPE_BYTE_BINARY);
      log.debug("w={},h={}", sourceImg.getWidth(), sourceImg.getHeight());
  ...
  private void calculateAndPublishArea(
      MouseEvent e, int x1, int x2, int y1, int y2, AutoTopologyDialog dialog) {
    Area area = new Area();
    ...
    for (int i = 0; i < dialog.thresholdImg.getWidth(); i++) {
      log.debug("get pixels i={}", i);
      MapTool.getFrame().updateDeterminateActionProgress(1);
      for (int j = 0; j < dialog.thresholdImg.getHeight(); j++) {
        if ((dialog.thresholdImg.getRGB(i, j) & 0xff) == 0) {
          Area leaf = new Area(new java.awt.Rectangle(i, j, 1, 1));
          tree.add(leaf);
        }
      }
    }

    // Area coercion is a performance boon.
    while (tree.size() > 1) {
      List<Area> tree2 = new ArrayList<Area>();
      for (int i = 0; i < tree.size(); i += 2) {
        log.debug("coerce tree.size={}, i={}", tree.size(), i);
        Area leaf = new Area();
        leaf.add(tree.get(i));
        if (i + 1 < tree.size()) leaf.add(tree.get(i + 1));
        tree2.add(leaf);
      }
      tree = tree2;
    }
    ...
    */
    ...
    trafo.concatenate(
        AffineTransform.getScaleInstance(
            (x2 - x1) / (double) dialog.thresholdImg.getWidth(),
            (y2 - y1) / (double) dialog.thresholdImg.getHeight()));
    area.transform(trafo);
    log.debug(
        "area.x={}, area.y={}, area.width={}, are.height={}",
        area.getBounds().x,
        area.getBounds().y,
