...
public class TokenTerrainModifierFunctions extends AbstractFunction {
  ...
  /**
   * @param parser
   * @param resolver the MapTool parser.
   ...
   */
  private Object getTerrainModifierInfo(VariableResolver resolver, List<Object> args)
      throws ParserException {
    ...
    if ("json".equals(firstParameter.toLowerCase())) {
      Token token = FunctionUtil.getTokenFromParam(resolver, "getTerrainModifier", args, 1, 2);
      return getTerrainModifierInfo(token);
    } else {
      Token token = FunctionUtil.getTokenFromParam(resolver, "getTerrainModifier", args, 0, 1);
   ...
   * @throws ParserException if an error occurs.
   */
  private Object setTerrainModifier(VariableResolver resolver, List<Object> args)
      throws ParserException {
    if (!MapTool.getParser().isMacroTrusted()) {
      throw new ParserException(
