...
public class TokenVisibleFunction extends AbstractFunction {
	...
	private TokenVisibleFunction() {
		super(0, 2, "setVisible", "getVisible", "setOwnerOnlyVisible", "getOwnerOnlyVisible", "setAlwaysVisible", "getAlwaysVisible");
	}
	...
	private Object setAlwaysVisible(Parser parser, List<Object> args) throws ParserException {
		...
		if (args.size() == 2) {
			token = FindTokenFunctions.findToken(args.get(1).toString(), null);
			if (token == null) {
				throw new ParserException(I18N.getText("macro.function.general.unknownToken", "setAlwaysVisible", args.get(1).toString()));
			}
		} else if (args.size() == 1) {
			MapToolVariableResolver res = (MapToolVariableResolver) parser.getVariableResolver();
			token = res.getTokenInContext();
			if (token == null) {
				throw new ParserException(I18N.getText("macro.function.general.noImpersonated", "setAlwaysVisible"));
			}
		} else if (args.size() == 0) {
			throw new ParserException(I18N.getText("macro.function.general.notEnoughParam", "setAlwaysVisible", 1, args.size()));
		} else {
			throw new ParserException(I18N.getText("macro.function.general.tooManyParam", "setAlwaysVisible", 2, args.size()));
		}
		val = args.get(0);
	...
	private Object getAlwaysVisible(Parser parser, List<Object> args) throws ParserException {
		...
		if (args.size() == 1) {
			token = FindTokenFunctions.findToken(args.get(0).toString(), null);
			if (token == null) {
				throw new ParserException(I18N.getText("macro.function.general.unknownToken", "getAlwaysVisible", args.get(0).toString()));
			}
		} else if (args.size() == 0) {
			MapToolVariableResolver res = (MapToolVariableResolver) parser.getVariableResolver();
			token = res.getTokenInContext();
			if (token == null) {
				throw new ParserException(I18N.getText("macro.function.general.noImpersonated", "getAlwaysVisible"));
			}
		} else {
			throw new ParserException(I18N.getText("macro.function.general.tooManyParam", "getAlwaysVisible", 1, args.size()));
		}
