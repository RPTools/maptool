...
public class LookupTableFunction extends AbstractFunction {
    ...
    public LookupTableFunction() {
        super(0, 5, "tbl", "table", "tblImage", "tableImage", "getTableNames", "getTableRoll", "setTableRoll", "clearTable", "addTableEntry", "deleteTableEntry", "createTable", "deleteTable",
                "getTableVisible", "setTableVisible", "getTableAccess", "setTableAccess", "getTableImage", "setTableImage", "copyTable", "setTableEntry");
    }
    ...
    private LookupTable getMaptoolTable(String tableName, String functionName) throws ParserException {
        ...
        if (!MapTool.getPlayer().isGM() && !lookupTable.getAllowLookup()) {
            if (lookupTable.getVisible()) {
                throw new ParserException(functionName + "(): " + I18N.getText("msg.error.tableUnknown") + tableName);
            } else {
                throw new ParserException(functionName + "(): " + I18N.getText("msg.error.tableAccessProhibited") + ": " + tableName);
            }
        }
        if (lookupTable == null) {
