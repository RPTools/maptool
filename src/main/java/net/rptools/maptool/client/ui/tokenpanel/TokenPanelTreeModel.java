...
public class TokenPanelTreeModel implements TreeModel, ModelChangeListener {
	...
	/**
	 * Accepts only tokens that are PCs and are owned by the current player (takes useStrictTokenManagement() into account).
	 */
	...
	/**
	 * Accepts only NPC tokens (for GM) or tokens owned by the current player (takes {@link ServerPolicy#useStrictTokenManagement()} into account). Here's the selection process:
	 * <ol>
	 ...
	 * <li>If the token is owned by the current player, return <code>true</code>. (Takes into account StrictTokenManagement and the AllPlayers ownership flag).
	 * <li>If the token is visible only to the owner, return <code>false</code>. (It's already been determined that we're not an owner.)
