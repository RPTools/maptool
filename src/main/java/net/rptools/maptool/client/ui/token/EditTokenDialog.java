...
public class EditTokenDialog extends AbeillePanel<Token> {
	...
	private static final ImageIcon REFRESH_ICON_ON = new ImageIcon(EditTokenDialog.class.getClassLoader()
			.getResource("net/rptools/maptool/client/image/refresh_arrows_small.png"));
	private static final ImageIcon REFRESH_ICON_OFF = new ImageIcon(EditTokenDialog.class.getClassLoader()
	...
	public void bind(final Token token) {
		...
		if (barPanel != null) {
			...
			for (int i = 0; i < bars.length; i += 2) {
				...
				if (token.getState(bar.getName()) == null) {
					...
					bar.setEnabled(true);
					bar.setValue((int) (TokenBarFunction.getBigDecimalValue(token.getState(bar.getName())).doubleValue()
							* 100));
				}
			}
		}
		...
		getSightTypeCombo().setSelectedItem(token.getSightType() != null ? token.getSightType()
		...
		if (heroLabData != null) {
			...
			if (isDirty && refreshDataButton.getIcon() != REFRESH_ICON_ON) {
				...
				refreshDataButton
						.setToolTipText("<html>Refresh data from Hero Lab<br/><b><i>Changes detected!</i></b></html>");
	...
	public boolean commit() {
		...
		if (barPanel != null) {
			...
			for (int i = 0; i < bars.length; i += 2) {
				...
				bar.setValue(
						(int) (TokenBarFunction.getBigDecimalValue(token.getState(bar.getName())).doubleValue() * 100));
			}
		}
		// Ownership
	...
	public void initOwnershipPanel() {
		...
		replaceComponent("ownershipPanel", "ownershipList", new JScrollPane(list,
	...
	public void initVblPreviewPanel() {
		...
		getAutoGenerateVblButton().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				vblPanel.setTokenVBL(
						TokenVBL.createVblArea(vblPanel.getToken(), (int) getAlphaSensitivitySpinner().getValue()));
				getTokenVblPanel().repaint();

			}
		});

		getClearVblButton().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (vblPanel.getTokenVBL() != null) {
		...
		getTransferVblToMap().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (vblPanel.getTokenVBL() != null) {
					...
					TokenVBL.renderVBL(MapTool.getFrame().getCurrentZoneRenderer(), vblPanel.getToken().getTransformedVBL(vblPanel.getTokenVBL()), false);
					// MapTool.getFrame().getCurrentZoneRenderer().getZone().tokenTopologyChanged();
		...
		getTransferVblFromMap().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Area mapVBL = TokenVBL.getMapVBL_transformed(MapTool.getFrame().getCurrentZoneRenderer(), vblPanel.getToken());

				vblPanel.setTokenVBL(mapVBL);
				vblPanel.setAutoGenerated(false);

				if (getCopyOrMoveCheckbox().isSelected()) {
					Area newTokenVBL = TokenVBL.getVBL_underToken(MapTool.getFrame().getCurrentZoneRenderer(), vblPanel.getToken());
		...
		getCopyOrMoveCheckbox().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (getCopyOrMoveCheckbox().isSelected()) {
					getTransferVblFromMap().setText(I18N.getString("token.properties.button.transferVblFromMap.move.text"));
					getTransferVblToMap().setText(I18N.getString("token.properties.button.transferVblToMap.move.text"));
				} else {
					getTransferVblFromMap().setText(I18N.getString("token.properties.button.transferVblFromMap.copy.text"));
					getTransferVblToMap().setText(I18N.getString("token.properties.button.transferVblToMap.copy.text"));
				}
		...
		getHideTokenCheckbox().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		...
		getAlphaSensitivitySpinner().addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				getTokenVblPanel().setAlphaSensitivity((int) getAlphaSensitivitySpinner().getValue());
	...
	public class HeroLabImageListRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
			...
			try {
				ImageIcon finalImage = ImageUtil.scaleImage(
						new ImageIcon(ImageManager.getImageAndWait(heroLabData.getImageAssetID(index))), 250, 175);
	...
	// //
	// MODELS
	private class TokenPropertyTableModel extends
			AbstractPropertyTableModel<net.rptools.maptool.client.ui.token.EditTokenDialog.TokenPropertyTableModel.EditTokenProperty> {
		...
		private List<TokenProperty> getPropertyList() {
			if (propertyList == null) {
				propertyList = MapTool.getCampaign()
