...
class PrefParts {
  ...
  /**
   * Map of control components, used for {@link #findComponent(String)} & {@link
   * #findComponent(String, Container)} layout changes
   */
  ...
  protected static final DeveloperOptionsDialog developerOptionsDialog =
      new DeveloperOptionsDialog();
  ...
  protected static final Map<Archetype, GridBagConstraints> CONSTRAINTS =
      new HashMap<>(); // section, group, prefLabel, prefControl, prefFave;

  /**
   * Method to generate the label and control for entries in {@link Classify.Collated}. For most
   * preferences everything is auto-go-matic, for things that need special creation, set the {@link
   * Classify.Collated#useDefaultControls} flag to false and add a branch to the switch statement.
   *
    ...
    if (!collated.useDefaultControls) {
      components =
          switch (collated) {
            case AUTH_KEY ->
                new Component[] {
                  Fn.createLabelUseKeys.apply(
                      "Preferences.label.auth.publicKey",
                      "Preferences.label.auth.publicKey.tooltip",
                      collated),
            ...
            case STARTUP ->
                new Component[] {
                  Fn.createLabelUseKeys.apply("Label.startup", null, collated), createConfigButton()
                };
            case THEME ->
                new Component[] {
                  Fn.createLabelUseKeys.apply("Label.themes", null, collated), createThemeButton()
                };
            case ICONS ->
                new Component[] {
                  Fn.createLabelUseKeys.apply(
                      "Label.iconTheme", "Preferences.label.iconTheme.tooltip", collated),
                  createThemeButton()
                };
            case MACRO_ED_THEME ->
                new Component[] {
                  Fn.createLabelUseKeys.apply(
                      "Label.theme.macroEditor", "Preferences.label.macroEditor.tooltip", collated),
                  createThemeButton()
                };
            default -> null;
          };
      ...
      components =
          new Component[] {
            Fn.createLabel.apply(null, null, collated), Fn.createComponent.apply(collated)
          };
        ...
        pref -> {
          ...
          try {
            if (pref.cast(Integer.class).isPresent()) {
              numberModel =
                  new SpinnerNumberModel(value.intValue(), min.intValue(), max.intValue(), 1);
            } else {
              numberModel =
                  new SpinnerNumberModel(
                      value.doubleValue(), min.doubleValue(), max.doubleValue(), 0.1);
            }
