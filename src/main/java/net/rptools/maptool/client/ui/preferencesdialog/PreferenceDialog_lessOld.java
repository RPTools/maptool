...
public class PreferenceDialog_lessOld { // } extends AbeillePanel<Object> {
  ...
  //                        .sorted(Comparator.comparing(collated ->
  // Classify.Collated.valueOf(collated.name()).ordinal()))
  ...
  //            PrefParts.PREF_PANE.add(sectionPanel,
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.SECTION));
  //            sectionRow++;
  ...
  //                    ));
  //            final Map<Classify.Group, List<Classify.Collated>> memberGroups =
  // members.stream().sorted(Comparator.comparing(collated ->
  // Classify.Collated.valueOf(collated.name()).ordinal())).collect(Collectors.groupingBy(collated
  // -> collated.group));
  //
  //            int groupRow = 0;
  ...
  //                groupRow++;
  //                sectionPanel.add(groupPanel,
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.GROUP));
  ...
  //                    try {
  //                        groupPanel.add(components[0],
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.LABEL));
  //                        groupPanel.add(components[1],
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.CONTROL));
  //                        groupPanel.add(new JCheckBox(),
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.FAVOURITE));
  ...
  // DefaultTreeTableModelII.createDefaultTreeTableModelII(root);
  ...
  //
  //    /**
  //     * The PreferenceDialog class represents a dialog window that allows users to customize
  // their
  //     * preferences.
  //     *
  ...
  //                ((JCheckBox)
  // PrefParts.findComponent(AppPreferences.mapLabelShowBorder.getKey()));
  ...
  //
  //    /**
  //     * Initializes and sets the initial state of various user preferences in the application.
  // This
  ...
  //                        Point p =
  // PrefParts.findComponent(components[1].getName()).getLocation();
  ...
  //
  //        private static final FlatSVGIcon.ColorFilter NEGATIVE_FILTER = new
  // FlatSVGIcon.ColorFilter().addAll(GraphicsUtil.INVERTED_ICON_COLOURS);
  //
  //        @Override
  ...
  //            }
  //            String stringValue = pTree.convertValueToText(node == null ? value : node, sel,
  // expanded, leaf, row, hasFocus);
  //            String tipText = pTree.convertValueToTooltipText(node == null ? value : node, sel,
  // expanded, leaf, row, hasFocus);
  ...
  //                Icon disabledIcon = sel ? laf.getDisabledSelectedIcon(tree, icon) :
  // laf.getDisabledIcon(tree, icon);
  ...
  //        }
  //
  //        public String convertValueToTooltipText(Object value, boolean selected, boolean
  ...
  //            Map<Classify.Group, List<Classify.Collated>> grouped =
  // sectionMembers.get(section).stream().collect(Collectors.groupingBy(collated ->
  // collated.group));
  //            List<PrefDataNode> groupedChildren = new ArrayList<>();
  //            for(Classify.Group group:
  // grouped.keySet().stream().sorted(Comparator.nullsFirst(Comparator.comparing(Classify.Group::toString))).toList()){
  //                System.out.println(group);
  //                List<PrefDataNode> children = new ArrayList<>();
  //                for(Classify.Collated collated: grouped.get(group)){
  //                    Component[] components = PrefParts.createComponentsFor(collated);
  //                    children.add(new PrefDataNode(collated.section, collated.group, (JLabel)
  // components[0], components[1], new JCheckBox(),collated, null));
  //                }
  //                groupedChildren.add(new PrefDataNode(section, group,null, null, null, null,
  // children));
  //            }
  //            sectionGroups.add(new PrefDataNode(section, null,null, null, null, null,
  // groupedChildren));
  //        }
  //        PrefDataNode myRootDataNode = new PrefDataNode(null, null,null,null,null, null,
  // sectionGroups);
  ...
  //        PrefParts.PREF_PANE.add(myTreeTable,
  // PrefParts.CONSTRAINTS.get(PrefParts.Archetype.SECTION));
  //    }
  //    protected static class PrefDataModel extends MyAbstractTreeTableModel {
  //        static protected String[] columnNames = {"Tree", "Section", "Group", "Label", "Control",
  // "Fave", "Collated", "Children"};
  //
  //        // Spalten Typen.
  //        static protected Class<?>[] columnTypes = {PrefDataModel.class, Classify.Section.class,
  // Classify.Group.class, Component.class, Component.class, Component.class,
  // Classify.Collated.class, PrefDataNode.class};
  //
  ...
  //
  //
  //        public PrefDataNode(Classify.Section section, Classify.Group group, JLabel label,
  // Component component, Component fave, Classify.Collated collated, List<PrefDataNode> children) {
