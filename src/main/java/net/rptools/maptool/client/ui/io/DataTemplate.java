 ...
 * 
 * <p>
 * The idea is that each subsystem will instantiate this class and pass it to the {@code LoadSaveImpl} registry. When the {@code LoadSaveImpl} needs to ask the subsystem for data, it will call the
 * {@code addDataObjects()} method.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The subsystem accomplishes this by overriding {@code prepareForDisplay()} and calling {@code addDataObjects()}. There are multiple definitions of the latter method so that the user can pass two
 * parameters and the compiler will figure out which to invoke.
 * </p>
 * 
 * <p>
 * On the application side, the user has passed on an instantiated object, so when we're ready, we call {@code prepareForDisplay(UIBuilder.Tree(model)} and pass the tree model, where it is stored
 * inside the object. When the {@code
 * addDataObjects()} is done it will return to {@code prepareForDisplay()}, which then returns to the application. The same object may be used again later.
 * </p>
 * 
 * <p>
 * The MapTool object being stored should have a {@code toString()} method that produces the desired display string on the UI. This isn't strictly necessary, because for {@code Map}s the String key is
 * used, but when the data structure is a {@code List} of some type, there is no key. This class copies all of the elements into a {@code Map} using the results of converting the object into a
 * {@c String} as the key. The strings are then sorted and added to the UI.
 * </p>
 * 
 * @author crash
 * 
