...
class FTPClientConn {
	...
	/**
	 * <p>
	 * Since this method uses a separate connection to the FTP server, the full pathname to the directory must be specified.
	 * </p>
	 * <p>
	 * Note that the directory separator may be "/" locally and in URLs, but FTP servers are not required to support it. This means that we really should be starting at the top of the tree and issuing
	 * <code>cd()</code> calls for the entire pathname, creating any that fail along the way. Too much work for now. FIXME
	 * </p>
	 * 
	 * @param dir
