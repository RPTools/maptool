...
public class PreferenceDialog extends AbeillePanel<Object> {
  ...
  static {
    ...
    for (Classify.Section sect : Classify.Section.values()) {
      ...
      final List<Classify.Collated> ungrouped =
          members.stream().filter(collated -> collated.group == null).toList();
      final List<Classify.Collated> grouped =
          members.stream().filter(collated -> collated.group != null).toList();
      final Map<Classify.Group, List<Classify.Collated>> memberGroups =
          grouped.stream()
              .sorted(
                  Comparator.comparing(
                      collated -> Classify.Collated.valueOf(collated.name()).ordinal()))
              .collect(Collectors.groupingBy(collated -> collated.group));
      ...
      for (Classify.Group g : memberGroups.keySet()) {
                ...
                BorderFactory.createTitledBorder(
                    g.equals(Classify.Group.NONE)
                        ...
                        : BorderFactory.createLineBorder(
                            UIManager.getDefaults().getColor("Window.border")),
        ...
        for (Classify.Collated collated : memberGroups.get(g)) {
          ...
          try {
            ...
            groupPanel.add(
                new JCheckBox(), PrefParts.CONSTRAINTS.get(PrefParts.Archetype.FAVOURITE));
    ...
    //        {
    //            static class JComponentRenderer extends JComponent implements TableCellRenderer,
    // UIResource {
    ...
    //                }
    //
    //                public void setValue(Object value) { if(value instanceof JComponent
    // jComponent){add(jComponent); }};
    //                @Override
    ...
    SEARCHABLE_BAR.setVisibleButtons(
        SearchableBarEx.SHOW_NAVIGATION
    ...
    SEARCHABLE_BAR.addPresetSearchValues(
        Arrays.stream(Classify.Section.values()).collect(Collectors.toSet()));
    SEARCHABLE_BAR
        .getSearchable()
        .addSearchableListener(
            searchableEvent -> table.setFilterText(searchableEvent.getSearchingText()));
  ...
  /// insufficient, such as adding listeners or the like. Custom creation should be handled in
  // {@link PrefParts#createComponentsFor(Classify.Collated)}
  ...
  private static class PrefTreeCellRenderer extends DefaultTreeCellRenderer {
    ...
    private static final FlatSVGIcon.ColorFilter NEGATIVE_FILTER =
        new FlatSVGIcon.ColorFilter().addAll(GraphicsUtil.INVERTED_ICON_COLOURS);
    ...
    public Component getTreeCellRendererComponent(
      ...
      String stringValue =
          pTree.convertValueToText(node == null ? value : node, sel, expanded, leaf, row, hasFocus);
      String tipText =
          pTree.convertValueToTooltipText(
              node == null ? value : node, sel, expanded, leaf, row, hasFocus);
      ...
      if (!tree.isEnabled()) {
        ...
        Icon disabledIcon =
            sel ? laf.getDisabledSelectedIcon(tree, icon) : laf.getDisabledIcon(tree, icon);
  ...
  private static class PreferenceTree extends FlatTree {
    ...
    /**
     * Called by the renderers to convert the specified value to text. This implementation returns
     * <code>value.toString</code>, ignoring all other arguments. To control the conversion,
     * subclass this method and use any of the arguments you need.
     *
    ...
    public String convertValueToTooltipText(
        Object value, boolean selected, boolean expanded, boolean leaf, int row, boolean hasFocus) {
      if (value instanceof DefaultMutableTreeNode node) {
        Object o = node.getUserObject();
  ...
  private class PreferenceTreeUI extends FlatTreeUI {
    ...
    protected void installDefaults() {
      ...
      try {
        ...
        paintLines.setBoolean(
            tree.getUI(),
            Boolean.parseBoolean(tree.getClientProperty("Tree.paintLines").toString()));
