...
public class NoteBookController {
  ...
  private void showEntry(NoteBookEntry entry) {
    ...
    if (entry == null) {
      ...
      if (entry.getType() == NoteBookEntryType.NOTE) {
        NoteEntry note = (NoteEntry) entry;
        AssetManager.getAssetAsynchronously(
            note.getNotesKey(),
            (key) -> {
              String noteText = AssetManager.getAsset(key).getDataAsString();
              Platform.runLater(() -> noteWebView.getEngine().loadContent(noteText));
            });
      } else {
        noteWebView.getEngine().loadContent("");
      }
