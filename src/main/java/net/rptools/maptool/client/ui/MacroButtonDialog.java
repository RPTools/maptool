...
public class MacroButtonDialog extends JDialog implements SearchListener {
	...
	public void show(MacroButton button) {
		...
		if (properties != null) {
			...
			if (allowEdits) {
				this.setTitle(properties.getLabel() + " - [" + I18N.getText("component.dialogTitle.macro.macroID") + ": " + id + "]");
	 ...
	 *
	 * This is trickier than it sounds as macOS doesn't automatically merge the per-window menu bar that this method used to create to the top-of-the-screen menu bar that is the stock in trade on
	 * macOS. So in order to create a portable method that works everywhere, we retrieve the "Edit" JMenu and add our elements to it. When the dialog closes, we call {@link #destroyMenuBar()} to
	 ...
	 *
	 * @param key
	 *            string key to lookup in the properties file (used to call {@link I18N#getKeystroke()} and {@link I18N#getText()}
	 * @param tb
