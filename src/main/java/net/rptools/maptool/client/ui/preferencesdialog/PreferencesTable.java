...
public class PreferencesTable extends CategorizedTable {
    ...
    private static final DefaultGroupTableModel DEFAULT_GROUP_TABLE_MODEL = new DefaultGroupTableModel(PREFERENCES_TABLE_MODEL);
    ...
    PreferencesTable(JideScrollPane tableScrollPane) {
        ...
        this.filterText.addListener(
                observable -> newRowFilter());

        CellRendererManager.registerRenderer(PreferencesTableModel.PrefComponentPane.class, new ComponentCellRenderer());
        CellEditorManager.registerEditor(PreferencesTableModel.PrefComponentPane.class, (CellEditorFactory) new ComponentCellEditor());
...
//        DefaultGroupRow
        for (int i = 0; i < PREFERENCES_TABLE_MODEL.getRowCount(); i++) {
            width = Math.max(width, fm.stringWidth(((JLabel) PREFERENCES_TABLE_MODEL.getValueAt(i, 2)).getText()));
            width1 = Math.max(width1, ((JComponent) PREFERENCES_TABLE_MODEL.getValueAt(i, 3)).getPreferredSize().width);
        }
        columnWidths.add(width + this.getRowMargin());
    ...
    public FontMetrics getFontMetrics(Font font) {
        ...
        if (this.isVisible()) {
            ...
            return MapTool.getFrame().getGraphicsConfiguration().getDevice().getDefaultConfiguration().createCompatibleVolatileImage(1, 1).getGraphics().getFontMetrics(font);
        }
    }
...
//                    hr.setToolTipText(
//                            ((PreferencesTableModel) jTable.getModel()).getColumnTooltipText(column));
                    return hr;
                };
...
//        this.prettify();
    ...
    private static class ComponentCellEditor extends AbstractCellEditor implements CellEditorFactory, TableCellEditor {
