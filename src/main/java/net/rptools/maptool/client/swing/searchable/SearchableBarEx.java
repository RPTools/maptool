...
public class SearchableBarEx extends JToolBar implements SearchableProvider {
    ...
    private static final ImageIcon ICON_HIGHLIGHT = RessourceManager.getSmallIcon(Icons.ACTION_TEXT_HIGHLIGHT);
    private static final ImageIcon ICON_NEXT = RessourceManager.getSmallIcon(Icons.STATUSBAR_RECEIVE_ON);
    private static final ImageIcon ICON_PREV = RessourceManager.getSmallIcon(Icons.STATUSBAR_TRANSMIT_ON);
    private static final ImageIcon ICON_RECURSIVE = RessourceManager.getSmallIcon(Icons.ACTION_RECURSIVE);
    private static final ImageIcon ICON_REGEX = RessourceManager.getSmallIcon(Icons.ACTION_TEXT_REGEX);
...
//     protected JComboBox<String> _comboBox;
    ...
    private static final Color DEFAULT_MISMATCH_BACKGROUND = ThemeSupport.getThemeColor(ThemeSupport.ThemeColor.YELLOW);
    ...
    @SuppressWarnings("unused")
     ...
     public SearchableBarEx(SearchableEx searchable, String initialText, boolean compact) {
         ...
         this._searchable.addSearchableListener(e -> {
             if (e.getID() == 3005 && SearchableBarEx.this._searchable.getSearchingText() != null && !SearchableBarEx.this._searchable.getSearchingText().isBlank()) {
    ...
    private void initComponents(String initialText) {
        ...
        final AbstractAction findNextAction = new AbstractAction() {
            public void actionPerformed(ActionEvent e) {
                ...
                } else if (found != -1 && SearchableBarEx.this._searchable.isRepeats() && found <= cursor) {
                    SearchableBarEx.this.select(found, text, false);
                    SearchableBarEx.this.setStatus(SearchableBarEx.this.getResourceString("SearchableBarEx.reachedBottomRepeat"), SearchableBarEx.this.getImageIcon("icons/repeat.png"));
                } else if (!SearchableBarEx.this._searchable.isRepeats() && found == -1) {
                    SearchableBarEx.this.setStatus(SearchableBarEx.this.getResourceString("SearchableBarEx.reachedBottom"), SearchableBarEx.this.getImageIcon("icons/error.png"));
                } else if (found != -1) {
                    SearchableBarEx.this.select(found, text, false);
                    SearchableBarEx.this.clearStatus();
        ...
        final AbstractAction findPrevAction = new AbstractAction() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (found == cursor) {
                    ...
                    SearchableBarEx.this.select(found, text, false);
                    SearchableBarEx.this.setStatus(SearchableBarEx.this.getResourceString("SearchableBarEx.reachedTopRepeat"), SearchableBarEx.this.getImageIcon("icons/repeat.png"));
                } else if (!SearchableBarEx.this._searchable.isRepeats() && found == -1) {
                    SearchableBarEx.this.setStatus(SearchableBarEx.this.getResourceString("SearchableBarEx.reachedTop"), SearchableBarEx.this.getImageIcon("icons/error.png"));
                } else if (found != -1) {
                    SearchableBarEx.this.select(found, text, false);
                    SearchableBarEx.this.clearStatus();
                }
        ...
        getSearchTextField().addFocusListener(new FocusAdapter() {
...
//        this._comboBox = this.createComboBox();
        DocumentListener listener = new DocumentListener() {
            private final Timer timer = new Timer(SearchableBarEx.this._searchable.getSearchingDelay(),
...
//        button.setRolloverSelectedIcon(this.getImageIcon("icons/highlightsRS.png"));
    ...
    protected AbstractButton createRepeatsButton() {
        ...
        button.addItemListener(e -> {
            if (e.getSource() instanceof AbstractButton) {
                SearchableBarEx.this.getSearchable().setRepeats(((AbstractButton)e.getSource()).isSelected());
            }
        });
        button.setOpaque(false);
        return button;
    ...
    protected AbstractButton createRegexButton() {
        ...
        button.addItemListener(e -> {
            if (e.getSource() instanceof AbstractButton) {
                SearchableBarEx.this.getSearchable().setWildcardEnabled(((AbstractButton)e.getSource()).isSelected());
            }
        });
        button.setOpaque(false);
        return button;
    ...
    protected AbstractButton createMatchCaseButton() {
        ...
        button.addItemListener(e -> {
            if (e.getSource() instanceof AbstractButton) {
                SearchableBarEx.this.getSearchable().setCaseSensitive(((AbstractButton)e.getSource()).isSelected());
                SearchableBarEx.this.addSearchingTextToHistory(SearchableBarEx.this.getSearchingText());
                SearchableBarEx.this.highlightAllOrNext();
            }
        });
    ...
    protected AbstractButton createWholeWordsButton() {
        ...
        button.addItemListener(e -> {
            if (e.getSource() instanceof AbstractButton && SearchableBarEx.this.getSearchable() instanceof WholeWordsSupport) {
                ((WholeWordsSupport)SearchableBarEx.this.getSearchable()).setWholeWords(((AbstractButton)e.getSource()).isSelected());
                SearchableBarEx.this.addSearchingTextToHistory(SearchableBarEx.this.getSearchingText());
                SearchableBarEx.this.highlightAllOrNext();
            }
        });
    ...
    protected void installComponents() {
        ...
        this._leadingLabel.setDisplayedMnemonic(this.getResourceString("Button.find.mnemonic").charAt(0));
        ...
        if ((this._visibleButtons & SHOW_WHOLE_WORDS) != 0 && this.getSearchable() instanceof WholeWordsSupport) {
    ...
    private void highlightAll() {
        ...
        if (text != null && !text.isBlank()) {
            ...
            if (index != -1) {
                ...
                this.setStatus(this.getResourceString("SearchableBarEx.notFound"), this.getImageIcon("icons/error.png"));
            }
    ...
    private void clearStatus() {
        ...
        getSearchTextField().setBackground(UIDefaultsLookup.getColor("TextField.background"));
        if (!this.isShowMatchCount() || getSearchTextField().getText().length() <= 0 && (getSearchTextField()).getText().length() <= 0) {
            this._statusLabel.setText("");
        } else {
            this._statusLabel.setText(this.getSearchable().getMatchCount() + " " + this.getResourceString("SearchableBarEx.matches"));
        }
        this.hideMessage();
    }
...
//        }
    ...
    public static SearchableBarEx install(SearchableEx searchable, KeyStroke keyStroke, SearchableBarEx.Installer installer) {
    ...
    public void setCompact(boolean compact) {
        this._compact = compact;
        this._findNextButton.setText(this._compact ? "" : this.getResourceString("SearchableBarEx.findNext"));
        this._highlightsToggle.setText(this._compact ? "" : this.getResourceString("SearchableBarEx.highlights"));
        this._findPrevButton.setText(this._compact ? "" : this.getResourceString("SearchableBarEx.findPrevious"));
    }
