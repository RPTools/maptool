 ...
 */
public class SearchableTree extends SearchableEx implements TreeModelListener, PropertyChangeListener {
    ...
    protected int getSelectedIndex() {
        if (this.isRecursive()) {
            ...
            return treePaths != null && treePaths.length > 0 ? this.getTreePaths().indexOf(treePaths[0]) : -1;
        } else {
    ...
    private void populateTreePaths0(Object node, TreePath path, TreeModel model) {
        if (this._tree.isRootVisible() || path.getLastPathComponent() != this._tree.getModel().getRoot()) {
        ...
        if (object instanceof TreePath) {
            ...
            if(treeNode instanceof DefaultMutableTreeNode dmtn){
                ...
                if(userObject instanceof SearchWords searchWords){
                    ...
                    textToSearch = localisedItem + " " + localisedItem.getValue() + localisedItem.getI18nKey().replace('.', ' ');
                }
            }
            if (!(this.getComponent() instanceof JTree tree)) {
                ...
                textToSearch += " " + tree.convertValueToText(treeNode, selected, tree.isExpanded((TreePath)object), tree.getModel().isLeaf(treeNode), tree.getRowForPath((TreePath)object), tree.hasFocus() && tree.getLeadSelectionPath() == object);
            }
        } else {
