  ...
  public void loadCampaignNoteBooks(PackedFile pakFile, NoteBookManager noteBookManager)
    ...
    for (String fname : files) {
      ...
      if (matcher.matches()) {
        ...
        throw new AssertionError("NoteBook filename pattern natching error."); // Shouldnt happen
      }
    }
    ...
    if (!dependencies.isEmpty()) {
      ...
      for (var dependency : dependencies) {
        dependenciesInfo.add(
            new NoteBookDependencyInfo(dependency.getNamespace(), dependency.getVersion()));
      }
