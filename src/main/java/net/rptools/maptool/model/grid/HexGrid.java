...
public abstract class HexGrid extends Grid {
  ...
  protected void createShape(double scale) {
    ...
    log.debug("create scaled cell shape");
  ...
  private GeneralPath createHalfShape(
    ...
    log.debug("creating half cell shape");
    GeneralPath hex = new GeneralPath();
  ...
  public void draw(ZoneRenderer renderer, Graphics2D g, Rectangle bounds) {
    ...
    //    g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, renderStyle.opacity));
    //    if (renderStyle.exposedOnly && renderer.getZone().hasFog()) {
    //      Area exposedArea =
    //          renderer.getZone().hasFog()
    //              ? renderer.getZoneView().getExposedArea(renderer.getPlayerView())
    //              : null;
    //      if (exposedArea != null) g.setClip(exposedArea);
    //    }
    // log.info("Draw grid - Store old RenderingHints.");
    RenderingHints oldHints = g.getRenderingHints();

    // log.debug("Draw grid - Set new RenderingHints.");
    ...
    if (this.getCellArea() == null) createGridArea(getSizeInPixels());
    if (this.scaledShape == null || lastScale != scale) {
      createScaledShape(scale);
      renderStyle.update(scale, (float) edgeLength);
    }
    GeneralPath finalShape = scaledShape;
    int offU = getOffU(renderer);
    int offV = getOffV(renderer);
    int count = 0;
    Color[] colours = renderStyle.getColours();
    BasicStroke[] strokes = renderStyle.getBasicStrokes();

    g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
    AffineTransform startAT = g.getTransform();
    ...
    for (double v = offV % (scaledMinorRadius * 2) - (scaledMinorRadius * 2);
      ...
      for (double u = start; u < end; u += incr) {
        currentAT.translate(0, u);
        // setGridDrawTranslation(g, u + offsetU, v);
        Shape clone = scaledShape.createTransformedShape(currentAT);
        finalShape.append(clone, false);
        // setGridDrawTranslation(g, -(u + offsetU), -v);
      }
    }
    kachunkachunk(g, finalShape, colours, strokes);
    g.setRenderingHints(oldHints);
    log.info("Draw grid done.");
  }

  public void kachunkachunk(Graphics2D g, Shape s, Color[] colours, BasicStroke[] strokes) {
    for (int i = 0; i < 5; i++) {
  ...
  protected Area createGridArea(int gridRadius) {
    ...
    final int hexRadius = gridRadius * getSizeInPixels();

    if (isHexHorizontal()) {
      gridArea.add(createHex(getSizeInPixels(), getSizeInPixels(), hexRadius, 0));
    } else {
      gridArea.add(createHex(getSizeInPixels(), -getSizeInPixels(), hexRadius, Math.toRadians(90)));
    }
  ...
  protected Area getScaledGridArea(Token token, int gridRadius) {
    gridRadius +=
        (int) (token.getFootprint(this).getBounds(this).getWidth() / getSizeInPixels() / 2);
