...
public class Token extends BaseModel implements Cloneable {
    ...
    /**
     * A state properties for this token. This allows state to be added that can change appearance
     * of the token.
     */
     ...
     * presentation of the token on-screen + the two notes fields is kept. Note that the sightType
     * is set to the campaign's default sight type, and the property type is not changed at all.
     * This will usually be correct since the default sight is what most tokens have and the
     * property type is probably specific to the campaign -- hopefully the properties were set up
    ...
    public void imported() {
         ...
         * propertyType, give a choice; or incorporate in the Campaign Properties window a marker
         * for what is default for new tokens.
         */
    ...
    /**
     * Set the name of this token to the provided string. There is a potential exposure of
     * information to the player in this method: through repeated attempts to name a token they own
     * to another name, they could determine which token names the GM is already using. Fortunately,
     * the showError() call makes this extremely unlikely due to the interactive nature of a
     * failure.
     *
     * @param name
     * @throws IOException
    ...
    /**
     * Lee: changing this to apply new X and Y values (as end point) for the token BEFORE its path
     * is computed. Path to be saved will be computed here instead of in ZoneRenderer
     */
    ...
    /**
     * Convert the token into a hash map. This is used to ship all of the properties for the token
     * to other apps that do need access to the <code>Token</code> class.
     *
