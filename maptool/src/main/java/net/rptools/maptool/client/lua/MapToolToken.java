 ...
 *
 */
public class MapToolToken extends LuaTable {
		...
		try {
			...
			switch (k.toLowerCase(Locale.ENGLISH)) {
				case LABEL: 
					...
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.label"));
					return;
				case NAME:
					if (isSelf || MapTool.getParser().isMacroTrusted()) {
					...
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.name"));
					return;
				case NOTES:
					if (isSelf || MapTool.getParser().isMacroTrusted()) {
						String note = null;
				...
				case OWNER_ONLY_VISIBLE:
					...
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.getOwnerOnlyVisible"));
					return;
				case GMNAME:
				case GM_NAME: 
				...
				case GMNOTES:
					...
					} else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.gmNotes"));
				case HALO:
					if (isSelf || MapTool.getParser().isMacroTrusted()) {
						TokenHaloFunction.getInstance().setHalo(token, value);
					}
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.halo"));
					return;
				case VISIBLE: 
					if (!MapTool.getParser().isMacroTrusted()) {
				...
				case INIT:
					...
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.init"));
					return;
				case INIT_HOLD:
					if (isSelf || MapTool.getParser().isMacroTrusted()) {
						TokenInitHoldFunction.getInstance().setTokenValue(token, isTrue(value));
					}
					else throw new ParserException(I18N.getText("macro.function.general.noPerm", "token.initHold"));
					return;
				case LAYER:
					if (!isSelf && !MapTool.getParser().isMacroTrusted()) {
...
//					return;
	...
	public LuaValue rawget(LuaValue key) {
		...
		try {
			...
			switch (k.toLowerCase(Locale.ENGLISH)) {
				case ID: return valOf(token.getId());
				case LABEL:
					if (!isSelf && !MapTool.getParser().isMacroTrusted()) {
...
//					return valueOf(token.getY());
	...
	public String tojstring() {
		if (token != null) return token.getId().toString();
		return super.tojstring();
	}
	
	@Override
	public LuaValue tostring() {
		if (token != null) return LuaString.valueOf(token.getId().toString());
