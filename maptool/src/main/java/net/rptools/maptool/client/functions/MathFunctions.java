...
public class MathFunctions extends AbstractFunction {
	...
	private MathFunctions() {
		super(0, UNLIMITED_PARAMETERS, "math.abs", "math.ceil", "math.floor", "math.cos", "math.cos_r", "math.sin",
				...
				"math.squareroot", "math.toRadians", "math.toDegrees", "math.pi", "math.e", "math.isOdd", "math.isEven",
				"math.isInt");
	}
	...
	private List<BigDecimal> getNumericParams(List<Object> param, int minParams, int maxParams, String functionName) throws ParserException {
		if (minParams == maxParams) {
			...
			throw new ParserException(I18N.getText("macro.function.general.notEnoughParam", functionName, minParams, param.size()));
		} else if (maxParams != UNLIMITED_PARAMETERS && param.size() > maxParams) {
			throw new ParserException(I18N.getText("macro.function.general.tooManyParam", functionName, maxParams, param.size()));
		}
		...
		for (Object o : param) {
			if (o instanceof BigDecimal) {
				...
				throw new ParserException(I18N.getText("macro.function.general.argumentTypeN", functionName, i, o.toString()));
			}
		}
	...
	public Object childEvaluate(Parser parser, String functionName, List<Object> param) throws ParserException {
		if ("math.abs".equals(functionName)) {
			...
			List<BigDecimal> nparam = getNumericParams(param, 1, 1, functionName);
			return BigDecimal.valueOf(Math.toRadians(Math.acos(nparam.get(0).doubleValue())));
			...
			List<BigDecimal> nparam = getNumericParams(param, 1, 1, functionName);
			return BigDecimal.valueOf(Math.toRadians(Math.asin(nparam.get(0).doubleValue())));
			...
			List<BigDecimal> nparam = getNumericParams(param, 1, 1, functionName);
			return BigDecimal.valueOf(Math.toRadians(Math.atan(nparam.get(0).doubleValue())));
			...
			return BigDecimal.valueOf(Math.toRadians(Math.atan2(nparam.get(0).doubleValue(), nparam.get(1).doubleValue())));
		} else if ("math.atan_r".equals(functionName)) {
			List<BigDecimal> nparam = getNumericParams(param, 2, 2, functionName);
			...
			List<BigDecimal> nparam = getNumericParams(param, 2, 2, functionName);
			return BigDecimal.valueOf(Math.hypot(nparam.get(0).doubleValue(), nparam.get(1).doubleValue()));
			...
			if (!isInteger(nparam.get(0))) {
				throw new ParserException(I18N.getText("macro.function.general.argumentTypeI", functionName, 1, nparam.get(0)));
			}

			if (!isInteger(nparam.get(1))) {
				throw new ParserException(I18N.getText("macro.function.general.argumentTypeI", functionName, 2, nparam.get(1)));
			}
			...
			if (param.size() > 0) {
				throw new ParserException(I18N.getText("macro.function.general.wrongNumParam", functionName, 0, param.size()));
			}
			...
			return nparam.get(0).remainder(BigDecimal.valueOf(2)).equals(BigDecimal.ZERO) ? BigDecimal.ONE : BigDecimal.ZERO;
			...
			return nparam.get(0).remainder(BigDecimal.valueOf(2)).equals(BigDecimal.ZERO) ? BigDecimal.ZERO : BigDecimal.ONE;
